...
class CategoryController extends Controller
{
    public function index()
    {
        try {
            ...
            foreach ($categorys as $category) {
                $postCounts = Post::with('categorys')->where('id', $category->id)->orderBy('id', 'desc')->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $validation = Validator::make($request->all(), [
                ...
                'category_position' => ['required', 'string', 'max:20', 'min:2'],
            ]);
            if ($validation->fails()) {
                return response()->json([
                    'success' => false,
    ...
    public function update(Request $request, $id)
    {
        try {
            $categorys = Category::findOrFail($id);
            $validation = Validator::make($request->all(), [
                'category_name' => ['string', 'max:20', 'min:10'],
                'category_position' => [ 'string', 'max:20', 'min:2'],
            ]);
            if ($validation->fails()) {
                return response()->json([
                    'success' => false,
